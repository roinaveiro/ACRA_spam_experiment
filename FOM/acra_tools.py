...
def getporbs(x, xp, obj, var, n, K = 1000):
...
def getResults(i,y_test,email,clf,ut,var,n,m):
    #tmpAcra = getAcraTime(email[[i], :],clf,ut,var,n)
    tmpSeq = getseqMCACRATime(email[[i], :],clf,ut,var,n, m)
    return ([ m,y_test[i], tmpSeq ])
       
def getParResults(y_test,email,clf,ut,var,n,m):
    inputs = range(email.shape[0])
    numCores = multiprocessing.cpu_count()
    result = Parallel(n_jobs=numCores)(delayed(getResults)(i, y_test, email, clf, ut, var, n, m) for i in inputs)   
    return np.array(result)
